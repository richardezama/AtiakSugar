...
function sendSmtpMail2($receiver_email, $receiver_name, $subject, $message)
{
    $mail = new PHPMailer(true);
    $general = GeneralSetting::first();
    ...
    try {
        $mail->isSMTP();
        $mail->Host       = "server01.i3c.co.ug";
        $mail->SMTPAuth   = true;
        $mail->Username   ="technomart@uncst.go.ug";
        $mail->Password   = "PKpHXwyK4B";
        $mail->Port       =587;
        $mail->CharSet = 'UTF-8';
        $mail->setFrom("technomart@uncst.go.ug");
        $mail->addAddress($receiver_email, $receiver_name);
        $mail->addReplyTo($general->email_from, "Rental Manager");
        $mail->isHTML(true);
        $mail->Subject = $subject;
        $mail->Body    = $message;
        $mail->send();
