...
@section('panel')
    ...
    <div id="addModal" class="modal fade" tabindex="-1" role="dialog">
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                ...
                <form action="{{ route('admin.estate.store')}}" method="POST">
                    @csrf
                    <div class="modal-body">
                    <div class="form-group">
                            <label class="form-control-label font-weight-bold">
                                 @lang('Type')</label>
                            <select name="estate_type" class="form-control select2-basic">
                                <option value="">@lang('Select Estate Type')</option>
                        ...
                        <div class="form-group">
                            <label class="form-control-label font-weight-bold"> 
                                @lang('No of Units')</label>
                            <input type="text" class="form-control" 
                            placeholder="@lang('Units')" name="units" required>
                        </div>
    ...
    <div id="editModal" class="modal fade" tabindex="-1" role="dialog">
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title"> @lang('Update Estate')</h5>
                    <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                        <span aria-hidden="true">&times;</span>
                    </button>
                </div>
    ...
    <div id="disableModal" class="modal fade" tabindex="-1" role="dialog">
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                ...
                <form action="{{ route('admin.estate.disable')}}" method="POST">
                    @csrf
                    <input type="text" name="id" hidden="true">
                    <div class="modal-body">
                        <p>@lang('Are you sure to delete') <span class="font-weight-bold type_name"></span> @lang('Estate')?</p>
                    </div>
                    <div class="modal-footer">
                        <button type="button" class="btn btn--dark" data-dismiss="modal">@lang('Close')</button>
    ...
    <script>
        (function ($) {
            ...
            $(document).on('click', '.editBtn', function () {
                ...
                var link    = `{{ route('admin.estate.update', '') }}/${data.id}`;
                var deckNumber = data.deck;
                modal.find('input[name=name]').val(data.name);
                modal.find('input[name=location]').val(data.location);
                modal.find('input[name=units]').val(data.units);
                modal.find('input[name=id]').val(data.id);
                modal.find('input[name=estate_type]').change(data.estate_type);
